...
import { getClients, saveClients, type Client } from '@/lib/adminStore'

const statusConfig = {
  actif: { label: 'Actif', color: 'bg-success/15 text-success', dot: 'bg-success' },
  impaye: { label: 'Impayé', color: 'bg-warning/15 text-amber-700', dot: 'bg-warning' },
...
const planConfig = {
  ...
  serenite: { label: 'Sérénité', price: '99 CHF', color: 'bg-primary-fixed text-primary' },
  accompagnement: { label: 'Accompagnement+', price: '165 CHF', color: 'bg-secondary-container text-secondary' },
}

type FilterStatus = 'tous' | Client['status']
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="space-y-3">
            {filtered.map((client, i) => {
              ...
              const pl = planConfig[client.plan]
              return (
                <motion.div
                  key={client.id}
                ...
                >
                  <div className="flex items-center gap-5">
                    ...
                    <div className="flex gap-1.5 shrink-0">
                      {client.status !== 'banni' && (
                        <motion.button whileTap={{ scale: 0.9 }} onClick={(e) => { e.stopPropagation(); updateStatus(client.id, 'banni') }}
                      ...
                      {client.status === 'banni' && (
                        <motion.button whileTap={{ scale: 0.9 }} onClick={(e) => { e.stopPropagation(); updateStatus(client.id, 'actif') }}
                          className="p-2.5 min-w-[44px] min-h-[44px] flex items-center justify-center rounded-lg bg-success/5 text-success hover:bg-success/15 transition-colors cursor-pointer" title="Réactiver">
                          <span className="material-symbols-outlined text-[18px]">check_circle</span>
                        </motion.button>
                      )}
                      {client.status === 'impaye' && (
                        <motion.button whileTap={{ scale: 0.9 }} onClick={(e) => { e.stopPropagation(); updateStatus(client.id, 'suspendu') }}
            ...
            >
              <div className="p-8">
                <div className="flex items-center justify-between mb-8">
                  <h2 className="font-[family-name:var(--font-heading)] text-xl font-bold">Détail client</h2>
                ...
                <div className="text-center mb-8">
                  ...
                  <p className="text-on-surface-variant text-sm">{selectedClient.phone}</p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-8">
                  <div className="bg-surface-low rounded-xl p-4 text-center">
                    ...
                    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-semibold ${statusConfig[selectedClient.status].color}`}>
                      <span className={`w-2 h-2 rounded-full ${statusConfig[selectedClient.status].dot}`} />
                      {statusConfig[selectedClient.status].label}
                    </span>
                  </div>
                  <div className="bg-surface-low rounded-xl p-4 text-center">
                    <p className="text-xs text-outline uppercase tracking-wider mb-1">Forfait</p>
                    <span className={`inline-block px-3 py-1 rounded-full text-sm font-semibold ${planConfig[selectedClient.plan].color}`}>
                      {planConfig[selectedClient.plan].label}
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-4 mb-8">
                  {[
                    ...
                    { icon: 'payment', label: 'Dernier paiement', value: new Date(selectedClient.lastPayment).toLocaleDateString('fr-FR') },
                    { icon: 'child_care', label: 'Enfants suivis', value: selectedClient.children.toString() },
                    { icon: 'payments', label: 'Montant mensuel', value: `${planConfig[selectedClient.plan].price}` },
                  ].map((d, i) => (
                ...
                {/* Notes */}
                <div className="mb-8">
                  <label className="text-xs font-semibold text-on-surface-variant uppercase tracking-wider mb-2 block">Notes internes</label>
                  <textarea
                    value={selectedClient.notes}
                    onChange={e => {
                      const updated = { ...selectedClient, notes: e.target.value }
                      setSelectedClient(updated)
                      updateNotes(updated.id, updated.notes)
                    }}
                    placeholder="Ajouter des notes sur ce client..."
                    className="w-full bg-surface-low rounded-xl p-4 text-sm outline-none resize-none h-24 focus:ring-2 focus:ring-primary/20 transition-all"
                  />
                </div>
                ...
                <div className="space-y-3">
                  ...
                  {selectedClient.status !== 'actif' && (
                    <button onClick={() => updateStatus(selectedClient.id, 'actif')}
                  ...
                  {selectedClient.status !== 'suspendu' && selectedClient.status !== 'banni' && (
                    <button onClick={() => updateStatus(selectedClient.id, 'suspendu')}
                  ...
                  {selectedClient.status !== 'banni' && (
                    <button onClick={() => updateStatus(selectedClient.id, 'banni')}
